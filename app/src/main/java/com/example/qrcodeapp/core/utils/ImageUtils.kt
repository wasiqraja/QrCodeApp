...
object ImageUtils {
    ...
    fun convertYuv420888ImageToBitmap(image: Image, luminosity: (Double) -> Unit): Bitmap {
        ...
        val yuvBytes = planes.map { plane ->
            ...
            val pixels = yuvBytes.map { it.toInt() and 0xFF }
    ...
    fun checkAndRotateBitmap(orientation: Int, bm: Bitmap): Bitmap {
        val bitmap: Bitmap = when (orientation) {
            90 -> rotateImage(bm, 90)!!
            180 -> rotateImage(bm, 180)!!
            270 -> rotateImage(bm, 270)!!
            0 -> bm
            else -> bm
        }
        return bitmap
